    ...
    def _build_fa_local(
        ...
        for i, base_addr in enumerate(base_addresses):
            ...
            if (
                ...
                parts.append(
                    self._stack_descs(addrs + kv_block_len, second_split, device_id)
                )
        return np.concatenate(parts) if parts else np.empty((0, 3), dtype=np.uint64)
    ...
    def _build_fa_remote(
        ...
        for i, base_addr in enumerate(nixl_agent_meta.kv_caches_base_addr):
            ...
            if self.transfer_topo.virtually_split_kv_in_blocks and not replicated:
                ...
                v_addrs = addrs + nixl_agent_meta.block_lens[i] // 2
                parts.append(self._stack_descs(v_addrs, second_split, device_id))
        return np.concatenate(parts) if parts else np.empty((0, 3), dtype=np.uint64)
