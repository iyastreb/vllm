...
class NixlBaseConnectorWorker:
    ...
    def _build_local_splits_from_plan(
        ...
        # Dense descriptors, as an Nx3 array or a list of (addr, len, dev).
        rows = (
            src_blocks_data.tolist()
            if isinstance(src_blocks_data, np.ndarray)
            else list(src_blocks_data)
        )
        has_ssm_descs = num_fa_descs < len(rows)
        ssm_idx = next(
        ...
        for p_idx, p_rank in enumerate(plan.all_source_ranks):
            ...
            for j, (addr, local_len, dev) in enumerate(rows):
                if j < num_fa_descs:
    ...
    def __init__(
        ...
        self._use_strided_descs = (
            hasattr(self.nixl_wrapper, "get_strided_xfer_descs") and not self._has_mamba
        )
    ...
    def _build_mamba_local(
        ...
        """Build desc region runs (conv sub-projections + ssm) per layer for
        local mamba blocks with DS conv layout, as an Nx5 uint64 array."""
        ...
        for i, base_addr in enumerate(base_addresses):
            ...
            for off, sz in conv_offsets:
                runs.append((base_addr + off, sz, device_id, page_stride, num_blocks))
            # SSM temporal state follows the conv state.
            runs.append(
                (base_addr + conv_size, ssm_size, device_id, page_stride, num_blocks)
            )
        return self._runs_array(runs)
    ...
    def _build_mamba_remote(
        ...
        """Build remote desc region runs (conv sub-projections + ssm) per layer.
        For hetero-TP, each D rank reads only its sub-projection slice from
        the P rank. Returns an Nx5 uint64 array."""
        assert self._conv_decomp is not None
        ...
        for i, base_addr in enumerate(nixl_agent_meta.kv_caches_base_addr):
            ...
            runs.append((ssm_addr, ssm_read_size, device_id, page_stride, num_blocks))
        return self._runs_array(runs)

    @staticmethod
    def _runs_array(runs: list[tuple[int, int, int, int, int]]) -> np.ndarray:
        """Pack strided runs (start, len, dev, stride, count) as an Nx5 uint64
        array, matching nixlStrideDesc's layout so NIXL memcpys it directly."""
        if not runs:
            return np.empty((0, 5), dtype=np.uint64)
        return np.array(runs, dtype=np.uint64)

    @staticmethod
    def _expand_runs(runs: np.ndarray) -> np.ndarray:
        """Expand Nx5 strided runs to dense Nx3 (addr, len, dev) descriptors,
        preserving run/block order (block b of a run -> start + b*stride)."""
        if runs.shape[0] == 0:
            return np.empty((0, 3), dtype=np.uint64)
        parts: list[np.ndarray] = []
        for start, length, dev, stride, count in runs:
            out = np.empty((int(count), 3), dtype=np.uint64)
            out[:, 0] = start + np.arange(count, dtype=np.uint64) * stride
            out[:, 1] = length
            out[:, 2] = dev
            parts.append(out)
        return np.concatenate(parts)

    def _xfer_descs_from_runs(self, runs: np.ndarray):
        """Build a NIXL xfer descriptor list from strided runs: a compressed
        strided list when supported, else dense Nx3 descriptors."""
        if self._use_strided_descs:
            return self.nixl_wrapper.get_strided_xfer_descs(
                runs, self.nixl_memory_type
            )
        return self.nixl_wrapper.get_xfer_descs(
            self._expand_runs(runs), self.nixl_memory_type
        )
    ...
    def _build_fa_local(
        ...
        """Build local FA descriptor runs for all layers as an Nx5 uint64 array
        (one strided run per region: addr, len, dev, stride, count)."""
        assert self.transfer_topo is not None
    ...
    def _build_fa_remote(
        ...
        """Build remote FA descriptor runs for all layers as an Nx5 uint64 array."""
        assert self.transfer_topo is not None
        fa_group_idx = next(
            i for i, t in enumerate(self._group_spec_types) if _is_attention_spec(t)
        )
        ...
        for i, base_addr in enumerate(nixl_agent_meta.kv_caches_base_addr):
            ...
            if self.transfer_topo.virtually_split_kv_in_blocks and not replicated:
                ...
                runs.append(
                    (
                        base_addr + rank_offset + nixl_agent_meta.block_lens[i] // 2,
                        second_split,
                        device_id,
                        page_size,
                        num_blocks,
    ...
    def register_local_xfer_handler(
        ...
        if self._has_mamba:
            assert self.num_descs == int(blocks_data[:, 4].sum())
        ...
        descs = self._xfer_descs_from_runs(blocks_data)
    ...
    def add_remote_agent(
        ...
        if (
            ...
            for handle_data in self._build_local_splits_from_plan(
                plan,
                self._expand_runs(self.src_blocks_data),
                self.num_descs,
            ):
